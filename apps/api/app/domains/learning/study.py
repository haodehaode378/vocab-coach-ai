...
from app.core.models import PracticeSession, ReviewLog, UserSetting, VocabItem
from app.core.services import get_or_create_user, get_or_create_user_settings, get_user_books
from app.core.sm2 import SrsState
from app.core.study_engine import evaluate_word
...
def get_books(user_email: str = "local@ai-vocab-agent.dev", db: Session = Depends(get_db)):
    ...
    setting = get_or_create_user_settings(db, user_email)
    total_words = sum(b["count"] for b in books)
    current_book_count = 0
    if setting.current_book_tag:
        current_book_count = next((b["count"] for b in books if b["tag"] == setting.current_book_tag), 0)
    return {
        "success": True,
        "data": {
            "books": books,
