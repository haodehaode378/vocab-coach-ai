...
def get_user_books(db: Session, user_id: int) -> list[dict]:
    results = (
        ...
        .filter(VocabItem.user_id == user_id, VocabItem.tags.isnot(None), VocabItem.tags != "")
        .group_by(VocabItem.tags)
        .order_by(VocabItem.tags.asc())
        .all()
    )
    ...
    for fp in file_paths:
        ...
        with path.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                # A word can exist in multiple books, so uniqueness must include tag.
                item = (
                    db.query(VocabItem)
                    .filter(
                        and_(
                            VocabItem.user_id == user.id,
                            func.lower(VocabItem.word) == word.lower(),
                            VocabItem.tags == source_tag,
                        )
                    )
                    .first()
                )
